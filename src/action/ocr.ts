...
Understood. Here's the revised prompt with **single string values** instead of arrays in the JSON:

---

**E-Way Bill and Challan/Invoice Number Extraction Prompt (For PDF)**
...
* Extract the **Challan No** or **Invoice No** from the document.
...
Return the extracted values in the following JSON format. If multiple values are found, return only the **first occurrence** of each.
...
Do not include any explanation, headers, or extra formatting—only return the JSON.
